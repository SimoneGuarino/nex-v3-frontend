...
export default function SurveyAppBootstrap({ children }: { children: React.ReactNode }) {
    const darkMode = loadFromLocalStorageBool("darkMode", false);

    const handoff = useMemo(() => readCrossAppLoading(), []);
    const [showSplash, setShowSplash] = useState(!!handoff);
