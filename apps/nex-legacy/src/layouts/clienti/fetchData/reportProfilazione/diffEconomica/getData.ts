//src\layouts\clienti\fetchData\reportProfilazione\diffEconomica\getData.ts
...
export function getData({
    ...
    const url = `${import.meta.env.VITE_API_CUSTOMERSFIDO}customers/report/diff-economica/list?ofs=${ofs}`;
    ...
    return FetchData(url, "POST", payload, abortController)
        ...
        .catch((error: any) => {
            if (error.name !== "AbortError") {
                ...
                console.error(error);
                let error_ =
                    "Sembra che ci sia stato un problema nel retrive dei dati nella tabella clienti con difficoltà economica, perfavore contatta un tecnico.";
