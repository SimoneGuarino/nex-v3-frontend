...
export function exportReportDiffEconomicaCSV({
    userContext,
    abortController,
    body,
    ChangeLoadStatus,
    ...
    const url = `${import.meta.env.VITE_API_CUSTOMERSFIDO}customers/report/diff-economica/export`;
    const payload = { ...body };
    ...
    return FetchFileData(url, {
        ...
        .then((res) => {
            ...
            if (res.kind === "blob") {
                ...
                link.href = blobUrl;
                link.download = res.filename || "report_difficolta_economica.csv";
