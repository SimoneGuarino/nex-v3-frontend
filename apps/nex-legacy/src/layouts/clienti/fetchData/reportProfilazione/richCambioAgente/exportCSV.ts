...
export function exportReportRichCambioAgenteCSV({
    userContext,
    abortController,
    body,
    ChangeLoadStatus,
    ...
    const url = `${import.meta.env.VITE_API_CUSTOMERSFIDO}customers/report/cambio-agente/export`;
    const payload = { ...body };
    ...
    return FetchFileData(url, {
        ...
        .then((res) => {
            ...
            if (res.kind === "blob") {
                ...
                link.href = blobUrl;
                link.download = res.filename || "report_cambio_agente.csv";
