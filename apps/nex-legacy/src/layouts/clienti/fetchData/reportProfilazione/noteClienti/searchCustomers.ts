...
export function searchCustomers({
    ...
    FetchData(
        `${import.meta.env.VITE_API_CUSTOMERSFIDO}customers/report/note-clienti/search`,
        "POST",
        body,
        abortController
    )
        ...
        .catch((error: any) => {
            if (error?.name !== "AbortError") {
                console.error(error);
                let msg =
                    "Si è verificato un errore nel recupero della lista clienti con note , riprova o contatta un tecnico.";
