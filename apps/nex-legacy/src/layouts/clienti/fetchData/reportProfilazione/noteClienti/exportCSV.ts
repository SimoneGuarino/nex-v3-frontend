...
export function exportReportNoteClientiCSV({
    userContext,
    abortController,
    body,
    ChangeLoadStatus,
    ...
    const url = `${import.meta.env.VITE_API_CUSTOMERSFIDO}customers/report/note-clienti/export`;
    const payload = { ...body };
    ...
    return FetchFileData(url, {
        ...
        .then((res) => {
            ...
            if (res.kind === "blob") {
                ...
                link.href = blobUrl;
                link.download = res.filename || "report_note_clienti.csv";
