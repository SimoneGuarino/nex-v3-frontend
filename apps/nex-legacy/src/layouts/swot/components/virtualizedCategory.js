...
const renderRow = ({ data, index, style }) => {
    ...
    return (
        <Typography key={dataSet[1]} component="li" {...dataSet[0]} noWrap style={inlineStyle}>
            {`${dataSet[1]}`}
        </Typography>
    );
};
...
function VirtualizedList(props) {
    ...
    const condition = data.label === 'famgilia' ?
        dataToInsert.gruppo ?
            ("famiglie" in Object.keys(dataToInsert.gruppo))
            : true
        : data.label === 'Gruppo' ?
            dataToInsert.linea ?
                Object.keys(dataToInsert.linea).find(elm => elm === "SubCategory") !== undefined ?
                    false
                    : true
                : true
            : data.label === 'Linea' ?
                dataToInsert.brand ?
                    Object.keys(dataToInsert.brand).find(elm => elm === "Categories") !== undefined ?
                        false
                        : true
                    : true
                : false
