...
export function GetUsersAPI({ abortController, setData, ChangeLoadStatus }: {
    abortController: any,
    setData: (prev: any) => void,
    ChangeLoadStatus: ({ from, bool }: { from: string, bool: boolean }) => void;
}): void {

    FetchData(`${import.meta.env.VITE_API_STOCKS}swot/users`, "GET", null, abortController)
        .then((res: any) => {
            if (res && Array.isArray(res) && res.length > 0) {
                setData(res);
            };
            ChangeLoadStatus({ from: 'users', bool: false });
        })
