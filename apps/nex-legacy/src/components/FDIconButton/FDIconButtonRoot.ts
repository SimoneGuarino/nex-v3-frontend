...
import type { FDColor, FDVariant, FDSize } from "./index";

type FDIconButtonOwnerState = {
  color: FDColor;
  variant: FDVariant;
  size: FDSize;
  circular?: boolean;
...
// Evitiamo che prop custom finiscano nel DOM
const shouldForwardProp = (prop: PropertyKey) =>
  prop !== "ownerState" && prop !== "bgColor";

const FDIconButtonRoot = styled(IconButton, { shouldForwardProp })<
  IconButtonProps & { ownerState: FDIconButtonOwnerState; bgColor: string }
>(({ theme, ownerState, bgColor }) => {
  ...
  const containedStyles = () => {
    ...
    const HoverbackgroundValue = "#0000000d";
...
export default FDIconButtonRoot;
