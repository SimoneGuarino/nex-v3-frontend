...
function navbar(theme: Theme, ownerState: NavbarOwnerState) {
  ...
  const { navbarBoxShadow } = boxShadows as any;
  const { rgba, pxToRem } = functions as any;
...
// navbarRow
const navbarRow = ({ breakpoints }: Theme, { isMini }: NavbarRowOwnerState) => ({
  display: "flex",
  alignItems: "center",
  justifyContent: "space-between",
  width: "100%",
